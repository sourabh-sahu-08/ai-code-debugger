...
export default function ChatInterface({ analysisId, initialMessages = [] }) {
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      toast.error("Failed to fetch AI assistant response.");
      setMessages((prev) => [
        ...prev,
        { role: "ai", content: "Apologies, a connection exception occurred. Please try again." },
      ]);
    } finally {
      setIsSending(false);
    }
        ...
        <AnimatePresence mode="popLayout">
          {messages.length === 0 ? (
            ...
            >
              ...
              <p className="text-xs mt-2 font-medium">Ask any technical follow-ups here</p>
            </motion.div>
          ) : (
            messages.map((msg, i) => (
              <motion.div
              ...
              >
                ...
                <div
                  className={`max-w-[80%] p-5 rounded-2xl text-sm leading-relaxed shadow-xl relative ${
                    msg.role === "user"
