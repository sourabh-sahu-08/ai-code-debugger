import { Check, Copy, AlertCircle, Zap, Clock, Maximize2, ShieldCheck, Activity, Brain } from "lucide-react";
...
export default function AnalysisResults({ data }) {
  ...
  const tabs = [
    { id: "fix", label: "Solution", icon: Zap, color: "text-blue-400" },
    { id: "explain", label: "Explanation", icon: Brain, color: "text-purple-400" },
    { id: "optimize", label: "Optimization", icon: Activity, color: "text-emerald-400" },
    { id: "security", label: "Security", icon: ShieldCheck, color: "text-orange-400" },
  ];
  ...
  return (
    <div className="flex flex-col h-full bg-slate-950/20 backdrop-blur-md">
        ...
        {tabs.map((tab) => (
          ...
          >
            <tab.icon
              size={16}
              className={`${
                activeTab === tab.id ? tab.color : "text-slate-600 group-hover:text-slate-400"
              } transition-colors`}
            />
            ...
            {activeTab === tab.id && (
              <motion.div
                layoutId="activeTab"
                className={`absolute bottom-1 w-6 h-0.5 rounded-full ${tab.color.replace(
                  "text-",
                  "bg-"
                )}`}
              />
      ...
      {/* Complexity and Confidence Indicator */}
            ...
            >
              ...
              {data.bestPractices && (
                <div className="space-y-4">
                  <h4 className="text-[10px] font-bold text-slate-600 uppercase tracking-widest ml-1">
                    Best Practices
                  </h4>
                  <div className="grid grid-cols-1 gap-2">
                    {String(data.bestPractices)
                      .split("\n")
                      .filter((l) => l.trim())
                      .map((line, i) => (
                        <div
                          key={i}
                          className="flex gap-3 text-[11px] text-slate-400 bg-[#0B0F1A] p-4 rounded-xl border border-white/5 hover:border-purple-500/20 transition-colors group"
                        >
              ...
              <div className="bg-emerald-500/5 p-5 rounded-2xl border border-emerald-500/10 flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-[12px] text-emerald-400/80 font-medium leading-relaxed">
                    Optimized to ensure minimized memory allocations and faster thread iterations.
                  </p>
                </div>
              </div>
            </motion.div>
              ...
              <div className="bg-[#0B0F1A] rounded-2xl border border-orange-500/10 p-6 space-y-6">
                ...
                <div className="space-y-4">
                  <p className="text-[13px] text-slate-300 leading-relaxed font-medium">
                    {data.securityAudit ||
                      "No major security risks detected. Basic compliance verified."}
                  </p>
                  <div className="flex flex-wrap gap-2 pt-2">
                    {["XSS-Safe", "Injection-Safe", "Secure-Dependencies"].map((tag) => (
                      <span
                        key={tag}
                        className="px-2 py-1 rounded-md bg-white/5 border border-white/5 text-[9px] font-bold text-slate-500 uppercase tracking-widest"
                      >
                        {tag}
                      </span>
                    ))}
