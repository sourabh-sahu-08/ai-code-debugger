...
import { ArrowRight, Eye, EyeOff, Lock, Mail, UserPlus, FileText } from "lucide-react";
...
export default function Register() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      playSound("success");
      toast.success(`Welcome to AI Code Review Nexus, ${data.user.name}!`);
      navigate("/");
    } catch (err) {
      playSound("error");
        ...
        <div className="mx-auto grid w-full max-w-[1200px] items-stretch gap-10 lg:grid-cols-2 lg:gap-16">
          ...
          >
            <div className="max-w-md">
              <p className="eyebrow mb-4 text-sky-400 text-glow">Start Free Sessions</p>
              <h1 className="text-3xl font-bold tracking-tight text-white leading-snug">
                Establish your private AI auditing account.
              </h1>
              <p className="mt-4 text-xs leading-relaxed text-slate-400">
                Join developers optimizing routines, resolving logical edge-cases, and managing detailed OWASP checklists.
              </p>
            </div>
            <div className="grid gap-3.5">
              {[
                "Deploy side-by-side public comparison diff sheets",
                "Synthesize in-browser haptic click feedback chords",
                "Custom HSL theme controls mapping color parameters"
              ].map((item) => (
                <div
                  key={item}
            ...
            <div className="mx-auto max-w-md">
              <div className="mb-10">
                ...
                <p className="mt-1 text-xs text-slate-500">
                  Register email coordinates to boot dashboard.
                </p>
              </div>
