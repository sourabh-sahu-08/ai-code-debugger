...
import BlackHole from '../components/visuals/BlackHole';
    ...
    <div className="min-h-screen w-full flex flex-col relative overflow-hidden bg-background">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-xl tracking-tight text-white">KhudSeKrle</span>
        </div>
        ...
        <div className="flex items-center gap-4">
          <Link to="/auth/login" className="text-sm font-medium text-text-muted hover:text-white transition-colors hidden sm:block">
            Log In
          </Link>
          <Link to="/auth/register">
            <Button size="sm" className="bg-white text-black hover:bg-white/90 border border-white">Get Started</Button>
          </Link>
        </div>
      </motion.nav>
      ...
      <main className="flex-1 flex flex-col mt-16 relative z-10">
          ...
          <div className="w-full grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-8 items-center z-10">
            ...
            >
              <motion.div variants={itemVariants} className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-border/50 bg-surface/30 backdrop-blur-sm mb-6 shadow-sm">
                ...
                <span className="text-sm font-medium text-white">KhudSeKrle v2.0 is live</span>
              </motion.div>
              ...
              <motion.h1 variants={itemVariants} className="text-5xl sm:text-6xl lg:text-7xl font-bold tracking-tight mb-6 leading-[1.1]">
                ...
                <span className="text-text-muted block">Start debugging.</span>
              </motion.h1>
              ...
              <motion.div variants={itemVariants} className="flex flex-col sm:flex-row items-center gap-4 w-full sm:w-auto">
                <Link to="/auth/register" className="w-full sm:w-auto">
                  <Button size="lg" className="w-full sm:w-auto text-base bg-white text-black hover:bg-white/90 group">
                    Start Debugging
                    <ArrowRight className="w-4 h-4 ml-2 group-hover:translate-x-1 transition-transform" />
                  </Button>
                </Link>
                <Link to="/debugger" className="w-full sm:w-auto">
                  <Button size="lg" variant="secondary" className="w-full sm:w-auto text-base border border-border/50 bg-transparent hover:bg-surface-soft text-white">
              ...
              <motion.div variants={itemVariants} className="mt-16 w-full max-w-3xl glass-card rounded-xl border border-border/50 p-6 flex flex-col sm:flex-row items-center justify-between gap-6 sm:gap-4 divide-y sm:divide-y-0 sm:divide-x divide-border/50">
                <div className="flex flex-col items-center sm:items-start w-full sm:px-4 pt-4 sm:pt-0 first:pt-0">
                  <span className="text-2xl lg:text-3xl font-bold text-white tracking-tight">10K+</span>
                ...
                <div className="flex flex-col items-center sm:items-start w-full sm:px-4 pt-4 sm:pt-0">
                  <span className="text-2xl lg:text-3xl font-bold text-white tracking-tight">95%</span>
                  <span className="text-[10px] sm:text-xs text-text-muted uppercase tracking-wider font-semibold mt-1">Resolution</span>
                </div>
                <div className="flex flex-col items-center sm:items-start w-full sm:px-4 pt-4 sm:pt-0">
                  <span className="text-2xl lg:text-3xl font-bold text-white tracking-tight">24/7</span>
            ...
            <div className="w-full h-full min-h-[400px] lg:min-h-[600px] relative flex items-center justify-center order-1 lg:order-2">
              ...
              <div className="absolute inset-0 z-10 pointer-events-none hidden sm:block">
                <motion.div 
                  ...
                  className="absolute top-[20%] right-[10%] glass-card px-4 py-2.5 rounded-lg flex items-center gap-2 border-border/50 text-white text-sm shadow-2xl bg-black/50"
                >
                  <Zap className="w-4 h-4 text-white" /> Bug Detected
                </motion.div>
                
                <motion.div 
                  animate={{ x: [0, 10, 0], y: [0, 5, 0] }} 
                  transition={{ duration: 11, repeat: Infinity, ease: "easeInOut", delay: 1 }}
                  className="absolute bottom-[25%] right-[20%] glass-card px-4 py-2.5 rounded-lg flex items-center gap-2 border-border/50 text-white text-sm shadow-2xl bg-black/50"
                >
                  <CheckCircle2 className="w-4 h-4 text-white" /> Root Cause Found
                </motion.div>
                
                <motion.div 
                  animate={{ y: [0, -10, 0], x: [0, -5, 0] }} 
                  transition={{ duration: 14, repeat: Infinity, ease: "easeInOut", delay: 2 }}
                  className="absolute top-[60%] left-[5%] glass-card px-4 py-2.5 rounded-lg flex items-center gap-2 border-border/50 text-white text-sm shadow-2xl bg-black/50"
                >
                  <Terminal className="w-4 h-4 text-white" /> <span className="font-mono">{`>_`} Line 42</span>
                </motion.div>
              </div>
            </div>
          </div>
        ...
        <section id="features" className="w-full max-w-7xl mx-auto px-6 py-24 border-t border-border/50">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-bold mb-4 text-white">Debug Smarter, Not Harder</h2>
        ...
        <footer className="w-full border-t border-border/50 bg-background pt-16 pb-8 mt-12">
          <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-lg text-white">KhudSeKrle</span>
            </div>
