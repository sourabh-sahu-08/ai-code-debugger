...
Return a single valid JSON object only. Do not wrap it in markdown block quotes (e.g. \`\`\`json).
  ...
  try {
    ...
    if (!code || !language) {
      return res.status(400).json({
        success: false,
        message: "Please provide both code and its programming language.",
      });
    }
    ...
    if (code.length > 15000) {
      return res.status(400).json({
        success: false,
        message: "Code snippet is too long. Please keep it under 15,000 characters.",
      });
    }
  ...
  try {
    ...
    if (analysis.userId.toString() !== req.user.id) {
      return res.status(401).json({
        success: false,
        message: "Not authorized to access this analysis report.",
      });
    }
...
exports.deleteAnalysis = async (req, res, next) => {
  try {
    ...
    if (analysis.userId.toString() !== req.user.id) {
      return res.status(401).json({
        success: false,
        message: "Not authorized to delete this analysis report.",
      });
    }
...
exports.chatWithAI = async (req, res, next) => {
  try {
    ...
    if (analysis.userId.toString() !== req.user.id) {
      return res.status(401).json({
        success: false,
        message: "Not authorized to chat about this analysis report.",
      });
    }
...
exports.togglePublic = async (req, res, next) => {
  try {
    ...
    if (analysis.userId.toString() !== req.user.id) {
      return res.status(401).json({ success: false, message: "Not authorized to alter visibility." });
    }
  ...
  try {
    ...
    if (!analysis || !analysis.isPublic) {
      return res.status(404).json({
        success: false,
        message: "Analysis report is private or does not exist.",
      });
    }
