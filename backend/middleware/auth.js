...
exports.protect = async (req, res, next) => {
  ...
  try {
    ...
    if (!req.user) {
      return res.status(401).json({ success: false, message: 'Access denied: User account not found.' });
    }
  ...
  return (req, res, next) => {
    if (!roles.includes(req.user.role)) {
      return res.status(403).json({
        success: false,
        message: `Forbidden: Role ${req.user.role} lacks appropriate access clearance.`,
      });
    }
    next();
  };
